...
IntArray_t Sorter::sort() {
	...
	try {
		...
		//Output?
		if(console_) {
			printArrayToConsole("Sorted array: ", array);
		}
	}
	catch(const char* e) {
...
void Sorter::parseCommandLine(int argc, char** argv) {
	...
	while ((opt = getopt(argc, argv, "a:o:cf:s:n:")) != -1) {
		switch (opt) {
			...
			default:
				...
				std::cout << "  -o              Output the sorted data to stdout." << std::endl;
				std::cout << "  -h              Displays this help information." << std::endl << std::endl;
				std::cout << "Examples: " << std::endl << std::endl;
				std::cout << "      isort -a radix -c -s 100000 -n 1000" << std::endl << std::endl;
				...
				std::cout << "      isort -a radix -c -f dataset.dat -s 100000 -n 1000" << std::endl << std::endl;
...
IntArray_t Sorter::readData() {
	...
	IntArray_t array(std::istream_iterator<FileLine>{ifs},
